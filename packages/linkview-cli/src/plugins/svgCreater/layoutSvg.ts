...
function calculateLineLen(layoutLine: LayoutLine, drowOptionsItem: DrawOptionsItem) {
  const len = layoutLine.map(({start, end}) => end - start + 1).reduce((total, cur) => total + cur);
  const gapCount = layoutLine.length - 1;
  const gapLen = drowOptionsItem.gap_length >= 1 ? drowOptionsItem.gap_length : drowOptionsItem.gap_length * len;
...
export default function layoutSvg(this: Options) {
  ...
  if (drawOptions && layout.length !== drawOptions.length) {
    warn(`${options.karyotype} and ${options.parameter} have different lines！You'd better check it carefully！`)
  }
